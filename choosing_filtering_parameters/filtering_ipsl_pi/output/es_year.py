...
is_plateau_year = np.zeros((dvolc.shape[0]))
for i in range(dvolc.shape[0]):
    if np.sum(is_plateau[i*12:(i+1)*12])>=6:
        is_plateau_year[i] = 1.0
...
pc = is_plateau_year
dvolc[np.abs(dvolc)>0.25] = 1.0
dvolc[np.abs(dvolc)<0.25] = 0.0

pc   = pc.astype(int)
volc = dvolc.astype(int)
...
print(EventSync(pc, volc, win_es))
