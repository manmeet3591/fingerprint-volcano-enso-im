...
def load_data():
    ...
    # load the data
    DATPATH = "../data/"
    fnino = DATPATH + "tos_Omon_HadCM3_past1000_r1i1p1_085001-185012_nino3_tseries.csv"
    dnino = np.genfromtxt(fnino, delimiter=",", dtype=float).flatten()-273.15
    fismr = DATPATH + "pr_Amon_HadCM3_past1000_r1i1p1_085001-185012_goswami_india_tseries.csv"
...
def plot(time, nino_dat, ismr_dat, c, N):
    ...
    ax1.plot(time, nino_anom,
             c="SteelBlue", label="Original", linewidth=1.0)
    ...
    for ax in [ax1, ax2]:
        ax.set_xlim(xlo, xhi)
        XMajorLocator = mdates.YearLocator(base=50, month=6, day=15)
...
def runmain():
    ...
    cutoff_num = np.arange(1., 25., 1.)
    cutoff_den = 12.
    filter_order = np.arange(1, 11)
