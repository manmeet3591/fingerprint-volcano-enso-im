...
def load_data(surr):
    ...
    # load the data
    DATPATH = "data_ipsl/"
    fnino = DATPATH + "tas_ipsl"+str(surr)+".csv"
    dnino = np.genfromtxt(fnino, delimiter=",", dtype=float).flatten()-273.15
    fismr = DATPATH + "pr_ipsl"+str(surr)+".csv"
...
def runmain():
    ...
    np.savetxt('data_ipsl/nino_phi'+str(surr)+'.txt',nino_phi)
    np.savetxt('data_ipsl/ismr_phi'+str(surr)+'.txt',ismr_phi)
    np.savetxt('data_ipsl/del_phi'+str(surr)+'.txt',del_phi)
    # # check plot
    # pl.clf()
    ...
    np.savetxt('data_ipsl/psync'+str(surr)+'.txt',psync)
