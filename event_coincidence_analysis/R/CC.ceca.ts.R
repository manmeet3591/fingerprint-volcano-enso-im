...
CC.ceca.ts <- function(seriesA,seriesB,seriesC,alpha=0.05,delT=0,delT.cond=0,sym=FALSE,sym.cond=FALSE,tau=0,tau.cond=0,sigtest="poisson",reps=1000){ 
    ...
    if(length(table(seriesBC))<2){
        ...
        names(CA_out)=c("null-hyp. precursor","null-hyp. trigger","p-value precursor","p-value trigger","conditioned precursor coincidence rate","conditioned trigger coincidence rate")
        return(CA_out)}
