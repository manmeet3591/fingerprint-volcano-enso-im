...
es_volc_mc = np.zeros((plv_hist.shape[0]))
...
#    print(EventSync(es_pi[i:i+taumax], es_volc[i:i+taumax], taumax)+EventSync(es_volc[i:i+taumax], es_pi[i:i+taumax],taumax))
    Q_hist_hv = EventSync(es_hist_mc[i:i+taumax], es_volc_mc[i:i+taumax], taumax)
    Q_hist_vh = EventSync(es_volc_mc[i:i+taumax], es_hist_mc[i:i+taumax],taumax)
