...
class monthdelta:
    ...
    def __str__(self):
        return '{0:d} month{1}'.format(self.__months,
                                       's' if abs(self.__months) != 1 else '')
    def __hash__(self):
...
def monthmod(start: date, end: date) -> (monthdelta, timedelta):
    ...
    This version of monthmod makes no allowance for aware datetime arguments
    with different tzinfo members.
    """
