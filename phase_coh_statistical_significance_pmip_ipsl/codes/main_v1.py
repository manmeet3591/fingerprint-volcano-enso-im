...
#max_slope = np.arange(0.1,0.15,0.05) 
max_slope = 0.1
#chunksize = np.arange(48,61,12)
chunksize = 48
#print(max_slope.shape[0],chunksize.shape[0])
#chunksize = 24
for imax in range(1):
	print("Entered imax loop")
	for jchunk in range(1):
		print("Entered loop")
		...
		is_plateau = utils.zero_slope(delphi_fit, chunksize = chunksize[jchunk], max_slope = max_slope[imax]) 
		...
		figs.delphi_timeseries(ct, del_phi, te, volc_time, dvolc, plv_time, plv, plv_99p, dvolc_source, delphi_5, delphi_95, delphi_fit, max_slope[imax], chunksize[jchunk])
