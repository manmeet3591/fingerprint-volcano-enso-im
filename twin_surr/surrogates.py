...
class Surrogates:
    ...
    def embed_time_series_array(self, time_series_array, dimension, delay):
        ...
        **Example:**
        ...
        if self.silence_level <= 1:
            print("Embedding all time series in dimension {dimension} and with lag {delay} ...")
    ...
    def twin_surrogates(self, original_data, dimension, delay, threshold,
        ...
        for i in range(N):
            #print(twins[i], len(twins[i])) # i corresponds to the series 
            # In the example dataset there are 200 data points in time and 190 twins in the first 
            # series
            twins_i_ = []
            for x in twins[i]:
                if x in twins_i_:
                    continue
                twins_i_.append(x)
            #print("removed duplicates",twins_i_, "length=", len(twins_i_))
            twins_i = twins_i_
            n_twins_i = len(twins_i_)
            #print(np.array(twins_i))
            flattened_twins_i = []

            #flatten the lis
            for x in twins_i:
                for y in x:
                    flattened_twins_i.append(y)
            #print(flattened_twins_i)
            k = randint(0,n_time-1)
            #k = int(np.floor(random.random() * n_time))

            j = 0
            while j < n_time:

                surrogates[i,j] = original_data[i,k]
                if k in flattened_twins_i:
                    #print("found a twin at index= ",k)
                    # Search for k in twins_i and then randomly change the value of 
                    # k accordingly
                    #for x in twins
                    k_twins = 0
                    k_twin_log = 0
                    for x in twins_i:
                        ...
                        if k_twin_log == 1:
                            break
                        k_twins = k_twins + 1
                    #print(twins_i[k_twins])           
                    #print(random.choice(twins_i[k_twins]))
                    k = random.choice(twins_i[k_twins])
                    #k = k + 1 # just for now

                else:
                    #print("did not find a twin at index = ",k)
                    k = k + 1
                #print("j = ",j, "surrogates[i,j]=",surrogates[i,j])
                if k >=n_time:
                    k = randint(0,n_time-1)
            ...
            #j = 0
            #surrogates = np.zeros(n_time)
            #ogc = original_data[0].copy(order='c')
            #while j < N:
            #    surrogates[j] = ogc[k]
    ...
    def test_threshold_significance(self, surrogate_function, test_function,
        ...
        """
        if self.silence_level <= 1:
            print("Starting significance test based on {realizations} realizations of surrogates...")
